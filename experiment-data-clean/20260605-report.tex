...
\begin{itemize}
    ...
    \item The best model, \textbf{XGBoost with feature reduction and a causal EMA
          smoother}, runs in real time and reaches \textbf{97.7\% accuracy} on held-out
...
\textbf{exponential moving average (EMA)} of the predicted probabilities is applied before
deciding: it weights recent windows more heavily and uses only current and past data, so it
runs in real time and needs only one running value per class (no buffer). This produces
stable predictions suitable for live deployment.

On the held-out sessions the smoothed real-time model reaches \textbf{97.7\% overall}
(Table~5), essentially matching the unsmoothed 98.1\% while removing the jitter. The small
gap is onset lag: the smoother takes a window or two to commit after a sample is introduced.
Almost all remaining errors fall in that onset window; once the response is established,
predictions are correct and stable.

\begin{table}[H]
\centering
\caption{Per-session accuracy of the deployed model (XGBoost, feature reduction,
causal EMA real-time smoothing).}
\begin{tabular}{lcr}
    \toprule
    \textbf{Session} & \textbf{Type} & \textbf{Accuracy} \\
    \midrule
    \texttt{blood\_9}      & Pure blood          & 98.5\% \\
    \texttt{sweat\_7}      & Pure sweat          & 98.3\% \\
    \texttt{mix\_75\_75\_3}  & Mixture, 50\% blood & 97.9\% \\
    \texttt{mix\_120\_30\_4} & Mixture, 20\% blood & 95.3\% \\
    \midrule
    \textbf{Overall} & (215 windows) & \textbf{97.7\%} \\
    \bottomrule
\end{tabular}
\end{table}
