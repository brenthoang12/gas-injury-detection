...
//
// 1 Hz sliding window: read sensors -> baseline-correct -> feature pipeline ->
//       trailing 60 s window -> tree model -> causal EMA -> class output every
//       second (Serial + LED). Mirrors test_exported_model.py test 2.
//
...
static void handleButton() {
    ...
    if (lastButton == HIGH && b == LOW) {
        ...
        if (measuring) {
            captureBaselines();
            feat_reset(); ema_init = false; lastMeasure = 0;
            digitalWrite(PIN_LED, LOW);
            Serial.println("=== MEASURING ===  (one prediction per second after the first 60 s)");
        } else {
            digitalWrite(PIN_LED, LOW);
...
void loop() {
    ...
    read_sensors(raw);
    feat_push(raw);                                // update histories every second

    if (feat_count < WIN_SIZE) {                   // trailing window not yet full (first 60 s)
        Serial.printf("[%lds] filling window %ld/%d\n", feat_count, feat_count, WIN_SIZE);
        return;
    }
    ...
    Serial.printf("[%lds] %-8s  %3.0f%%   (base %.2f / sweat %.2f / blood %.2f)\n",
                  feat_count, CLASS_NAME[pred], ema[pred] * 100.0f, ema[0], ema[1], ema[2]);
