...
ETOH_SENSITIVITY_CODE = 21.5   # nA/ppm  (from sensor)
ETOH_TIA_GAIN         = 249.0     # kΩ
...
def compute_etoh_ppm(df: pd.DataFrame) -> pd.Series:
    vref = df["etoh_vref"].mean()           # fixed Vref from session average
    return ((df["etoh_vgas"] - vref) / M_ETOH).clip(lower=0)
...
def evaluate_offset(path: str = CLEAN_AIR_PATH,
    ...
    df["etoh_ppm_raw"] = compute_etoh_ppm(df)
 
    # 3. Reject outliers
    clean_mask         = iqr_mask(df["etoh_ppm_raw"], k=iqr_k)
    df["etoh_clean"]    = df["etoh_ppm_raw"].where(clean_mask)
    n_outliers         = (~clean_mask).sum()
    print(f"Outliers rejected: {n_outliers} / {len(df)}  ({100*n_outliers/len(df):.1f}%)")
 
    # 4. Compute offset candidates
    clean_vals = df["etoh_clean"].dropna()
    ...
    print(f"  ETOH_OFFSET_PPM = {median_val:.4f}  # median of clean-air baseline")
    ...
    ax1.plot(df["time_s"], df["etoh_ppm_raw"], color="#378ADD", lw=0.8, alpha=0.7, label="etoh_ppm (raw)")
    ax1.scatter(df.loc[~clean_mask, "time_s"], df.loc[~clean_mask, "etoh_ppm_raw"],
    ...
    corrected = (df["etoh_ppm_raw"] - median_val).clip(lower=0)
    ax3.plot(df["time_s"], corrected, color="#1D9E75", lw=0.8)
    ax3.axhline(0, color="#888780", lw=0.8, ls="--")
    ax3.set_xlabel("time (s)")
    ax3.set_ylabel("etoh_ppm (offset applied)")
    ax3.set_title(f"after offset subtraction (offset = {median_val:.2f})")
 
    plt.suptitle("ETOH offset evaluation — clean air recording", fontsize=12, y=1.01)
    plt.tight_layout()
    plt.show()
